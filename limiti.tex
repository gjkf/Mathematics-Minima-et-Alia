...
Possiamo quindi dire che er un numero $\varepsilon$ positivo
\begin{equation*}
...
\begin{definizioneLimiteFinito}
	Sia $f$ una funzione definita in un intorno $I$ del punto $x_0$, esnza che sia necessariamente
	definita in $x_0$.\\
	...
	\lim\limits_{x\to x_0} f(x) &= l\\
	\ArrowBetweenLines
	\forall \varepsilon>0, \exists\,\delta_\varepsilon>0\mid\forall x: 0<\vert x-x_0\vert 
...
\begin{definizioneLimiteInfinito1}
	...
	\lim\limits_{x\to x_0} f(x) &= l\\
	\ArrowBetweenLines
	\forall k>0, \exists\,\delta_M>0\mid\forall x: 0<\vert x-x_0\vert &< \delta_M \Rightarrow
	f(x) > M
	\end{align*}
...
\begin{definizioneLimiteInfinitoFinito1}
	...
	\lim\limits_{x\to x_0} f(x) &= l\\
	\ArrowBetweenLines
	\forall \varepsilon>0, \exists\,k_\varepsilon>0\mid\forall x: x>&k_\varepsilon \Rightarrow
...
\begin{definizioneLimiteFinitoDestro}
	...
	\lim\limits_{x\to x_0} f(x) &= l\\
	\ArrowBetweenLines
	\forall \varepsilon>0, \exists\,\delta_\varepsilon>0\mid\forall x: 0< x-x_0	&<\delta_\varepsilon 
...
\begin{definizioneLimiteFinitoSinistro}
	...
	\lim\limits_{x\to x_0} f(x) &= l\\
	\ArrowBetweenLines
	\forall \varepsilon>0, \exists\,\delta_\varepsilon>0\mid\forall x: -\delta_\varepsilon< x-x_0&<0
	\Rightarrow \vert f(x) - l\vert < \varepsilon
	\end{align*}
...
\end{equation*}
Per trovare questo limite, possiamo molto semplicemente immaginare il grafico del logaritmo in base $2$
\begin{center}
	\begin{tikzpicture}
	...
	\end{tikzpicture}
\end{center}
e osservare il suo comportamento man mano che si avvicina da sinistra a $4$. Notiamo subito che tende
a crescere e continuerà fino all'infinito. Quindi
\begin{equation*}
\lim\limits_{x\to4^-}\log_2 x-2 = +\infty
\end{equation*}
Immaginiamo ora di sostituire $\log_2 x-2$ con $+\infty$ nella nostra funzione. Da notare che non 
usiamo $=$ ma $\to$.
\begin{equation*}
\lim\limits_{x\to4^-}\frac{1}{\log_2 x -2}\to\lim\limits_{x\to4^-}\frac{1}{+\infty} = 0
\end{equation*}
Abbiamo quindi scoperto che il limite della nostra originali funzione è $0$.\\\\
