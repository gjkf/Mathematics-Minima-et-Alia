...
Per il \hyperref[enum:ex:aff:2:1]{punto uno} dobbiamo vedere in che caso l'equazione non
...
Una sfera può essere immaginata come una semicirconferenza che ruota attorno all'asse delle 
ascisse. Questo significa che, centrando la circonferenza in $O(0,0)$ e definendo $r$ il suo
raggio, si ha
\begin{center}
  \begin{tikzpicture}
    \begin{axis}[xmin=-2,ymin=-1,xmax=2,ymax=2, xtick={-1,1}, xticklabels={$-r$,$r$},
      yticklabels={,,}]
      \coordinate (O) at (0,0);
...
Andremo a dimostrare prima il caso più generale, il tronco di cono. Un tronco di cono non è altro 
che una retta che è ruotata attorno all'asse delle ascisse. Ad esempio
\begin{center}
  \begin{tikzpicture}
    \begin{axis}[xmin=-1,ymin=-1,xmax=3,ymax=2, ytick={1},yticklabels={$r$}]
      \coordinate (O) at (0,0);
      \node[fill=white,circle,inner sep=0pt] (O-label) at ($(O)+(-135:10pt)$) {$O$};
      \addplot[domain=-1:3] {x/2+1};
      \LabelPoint[mark=*][below right]{2}{2}{$(h,R)$}
    \end{axis}
  \end{tikzpicture}
\end{center}
...
Dato che la circonferenza non è un luogo geometrico descrivibile da una funzione, possiamo 
